...
def subscribe():
    ...
    trials = conn.execute("""
        SELECT plan_id FROM trials WHERE coach_id = ?
    """, (session["coach_id"],)).fetchall()

    conn.close()

    used_trial_plan_ids = [t["plan_id"] for t in trials]
...
def activate_trial(plan_id):
    ...
    existing_trial = conn.execute("""
        SELECT * FROM trials WHERE coach_id = ? AND plan_id = ?
    """, (session["coach_id"], plan_id)).fetchone()

    if existing_trial:
        conn.close()
        return jsonify({
            "success": False,
            "message": "شما قبلاً از تست رایگان این پلن استفاده کرده‌اید."
        }), 403
